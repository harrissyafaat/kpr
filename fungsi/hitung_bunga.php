...
$listAnggota = mysqli_query ($koneksi, "SELECT kode_anggota FROM t_pinjam GROUP BY kode_anggota");
while ($row = $listAnggota->fetch_assoc()) {
	// Hitung Bunga Pinjam
	...
	if ($rselisih_pinjam['selisih'] < 30){
		$bp = ceil(0.005 * $rselisih_pinjam['selisih'] * $rsaldo_pinjam['saldo']);
		echo $bp;
		echo "<br>";
		echo $rselisih_pinjam['selisih'];
		echo "<br>";
		echo $rsaldo_pinjam['saldo'];
		$qBunga_pinjam = mysqli_query ($koneksi, "INSERT INTO t_pinjam VALUES('', '$row[kode_anggota]', '1234', CURDATE(), '$bp', '0', '0', '0', '0', 'admin', CURDATE(), '0')");
	} else {
		// Insert Bunga Pinjam
		$bp = ceil(0.005 * $sel_tgl_hitung * $rsaldo_pinjam['saldo']);
		$qBunga_pinjam = mysqli_query ($koneksi, "INSERT INTO t_pinjam VALUES('', '$row[kode_anggota]', '1234', CURDATE(), '$bp', '0', '0', '0', '0', 'admin', CURDATE(), '0')");
	}
}
